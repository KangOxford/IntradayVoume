...
# def get_universal(start_index, num_of_stocks, end_index):
    ...
    for index in tqdm(range(111)):
        ...
        from sklearn.cluster import KMeans
        # def balanced_kmeans(X, n_clusters, max_iter=300, n_init=10):
        #     kmeans = KMeans(n_clusters=n_clusters, max_iter=max_iter, n_init=n_init)
        #     kmeans.fit(X)
        #
        #     sizes = np.bincount(kmeans.labels_)
        #     while np.any(sizes < X.shape[0] / n_clusters):
        #         # If any cluster is too small, re-run k-means
        #         kmeans = KMeans(n_clusters=n_clusters, max_iter=max_iter, n_init=n_init)
        #         kmeans.fit(X)
        #         sizes = np.bincount(kmeans.labels_)
        #
        ...
        num_clusters = 20
        X_KM = features.iloc[index*bin_size:index * bin_size + train_size].T
        ...
        balanced_kmeans(X, num_clusters)

        # Fit the KMeans model to the data and get the cluster assignments
        clusters = kmeans.fit_predict(X_KM)

        # Assign the cluster assignments to a new column in your DataFrame
        X_KM['cluster'] = clusters
        X_KM['cluster'] = labels
        X_KM = X_KM.reset_index()
        for cluster_index in range(num_clusters):
            cluster_1_indices = X_KM.index[X_KM['cluster'] == cluster_index].tolist()
